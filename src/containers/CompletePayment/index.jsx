...
export function CompletePayment() {
    ...
    useEffect(() => {
        ...
        if (!clientSecret) {
            setError("Client secret não encontrado");
            return;
        }
        ...
        return (
            <div className="container">
                <div id="payment-status">
                    <div id="status-icon" style={{ backgroundColor: STATUS_CONTENT_MAP.default.iconColor }}>
                        {STATUS_CONTENT_MAP.default.icon}
                    </div>
                    <h2 className="status-title">{error}</h2>
                    <a id="retry-button" href="/user/carrinho">Tentar novamente</a>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="container">
            <div id="payment-status">
                <div id="status-icon" style={{ backgroundColor: STATUS_CONTENT_MAP[status]?.iconColor || STATUS_CONTENT_MAP.default.iconColor }}>
                    {STATUS_CONTENT_MAP[status]?.icon || STATUS_CONTENT_MAP.default.icon}
                </div>
                <h2 id="status-text" className="status-title">
                    {STATUS_CONTENT_MAP[status]?.text || STATUS_CONTENT_MAP.default.text}
                </h2>
                {intentId && (
                    <>
                ...
                <a 
                    id="retry-button" 
                    href={STATUS_CONTENT_MAP[status]?.url || STATUS_CONTENT_MAP.default.url}
                >
                    {STATUS_CONTENT_MAP[status]?.buttonText || STATUS_CONTENT_MAP.default.buttonText}
                </a>
            </div>
        </div>
    );
